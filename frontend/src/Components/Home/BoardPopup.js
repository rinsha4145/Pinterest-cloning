...
const Popup = ({postid,isBoardMenuVisible}) => {
  ...
  const savetoboard=handleAsync(async(id)=>{
    const response=await axiosInstance.post(`/addtoboarad`,{boardId:id,postId:postid})
    const savedData = response.data.saved;
    console.log(savedData)
    handleAsync(fetchData)()
    setShow(false)
      ...
      {show && (
        ...
        {filteredData.length > 0 ? (
          filteredData.map((item) => (
            <div className="flex items-center mt-2 p-2 hover:bg-gray-100 cursor-pointer rounded-md" onClick={()=>savetoboard(item._id)}>
  ...
  <path fillRule="evenodd" d="M19.5 21a3 3 0 0 0 3-3V9a3 3 0 0 0-3-3h-5.379a.75.75 0 0 1-.53-.22L11.47 3.66A2.25 2.25 0 0 0 9.879 3H4.5a3 3 0 0 0-3 3v12a3 3 0 0 0 3 3h15Zm-6.75-10.5a.75.75 0 0 0-1.5 0v4.19l-1.72-1.72a.75.75 0 0 0-1.06 1.06l3 3a.75.75 0 0 0 1.06 0l3-3a.75.75 0 1 0-1.06-1.06l-1.72 1.72V10.5Z" clipRule="evenodd" />
          ...
          <div className="p-4  relative h-[200px] overflow-y-auto">
            {boards?.map((board)=>(
            <div className="flex items-center mt-2 p-2 hover:bg-gray-100 cursor-pointer rounded-md" onClick={()=>savetoboard(board._id)}>
              <div className="bg-gray-200 w-8 h-8 rounded-md"></div>
          ...
          <div className="p-4">
            ...
            <div className="flex items-center mt-2 p-2 hover:bg-gray-100 cursor-pointer rounded-md">
              ...
              <button className="ml-2 text-sm" onClick={handleboard} >Create board</button>
            </div>
          </div>
        </div>
      )}

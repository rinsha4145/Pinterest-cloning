...
const addToBoard = async (req, res, next) => {
  ...
  return res.status(200).json({message: "Post already saved and added to the board",board: updatedBoard,saved: save });
};
...
const removeFromBoard = async (req, res, next) => {
  ...
  res.status(200).json({message: "Post successfully removed from the board",board: updatedBoard,});
};

...
const getpostbyid = async (req, res, next) => {
  const onepost = await Posts.findById(req.params.id).populate(
    "category owner comments.user"
  );
  if (!onepost) {
...
const getbycategory = async (req, res, next) => {
  ...
  const posts = await Posts.find({ category: categoryDoc._id }).populate(
    "category"
  );
  if (!categoryDoc) {
...
const addPost = async (req, res, next) => {
  ...
  res
    .status(200)
    .json({ status: "success", message: "Post added successfully", newPost });
};
...
const postUpdate = async (req, res, next) => {
  ...
  const updatedPost = await Posts.findByIdAndUpdate(req.params.id, value, {
    new: true,
  });
  if (!updatedPost) {

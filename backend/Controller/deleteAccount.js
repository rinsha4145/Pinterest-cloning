...
const deleteAccount = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Account deletion scheduled, confirmation email sent', deletionDate });
  } catch (error) {
...
const confirmDeletion = async (req, res) => {
  ...
  try {
    const user = await User.findOne({ deletionToken, deletionTokenExpiry: { $gt: Date.now() } });
    if (!user) {
    ...
    res.status(500).json({ message: 'Error confirming account deletion', error });
  }
};
...
const cancelDeletionOnLogin = async (req, res) => {
  ...
  try {
    ...
    if (user.deletionScheduled) {
      ...
      return res.status(200).json({ message: 'Account deletion canceled and email sent.' });
    }

    return res.status(200).json({ message: 'No deletion scheduled for this account.' });
  } catch (error) {

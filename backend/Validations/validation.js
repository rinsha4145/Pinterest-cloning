...
const userValidationSchema = Joi.object({
  ...
  birthdate: Joi.date().less("now"), // Ensure the birthdate is in the past,
  ...
  stateProvinceRegion: Joi.string().messages({
  ...
  admin: Joi.boolean().default(false).optional().messages({
  ...
  commented: Joi.array()
    ...
    .messages({
      ...
      "string.length":
        "Commented posts should contain valid ObjectId references.",
    }),
...
const postValidationSchema = Joi.object({
  ...
  category: Joi.string().messages({
    "any.only":
      "Category must be one of the following: Food, Fashion, Travel, DIY, Tech, Home, Other",
  ...
  likesCount: Joi.number().min(0).default(0).messages({
    "number.min": "Likes count cannot be negative",
  }),

  commentsCount: Joi.number().min(0).default(0).messages({
    "number.min": "Comments count cannot be negative",
  }),

  savesCount: Joi.number().min(0).default(0).messages({
    "number.min": "Saves count cannot be negative",
  }),

  viewsCount: Joi.number().min(0).default(0).messages({
